...
class KnowledgeDatabase:
    ...
    # ========================================================================
    # REX UNLIMITED MEMORY METHODS
    # ========================================================================
    
    def store_conversation_session(self, session: 'ConversationSession') -> int:
        """Store or update a conversation session"""
        cursor = self.connection.cursor()
        cursor.execute("""
            INSERT INTO conversation_sessions 
            (session_id, title, workspace_path, model_id, created_at, last_message_at, 
        ...
        """, (session.session_id, session.title, session.workspace_path, session.model_id,
              session.created_at, session.last_message_at, session.message_count,
              session.total_tokens, session.is_active))
    ...
    def store_conversation_message(self, message: 'ConversationMessage') -> int:
        ...
        cursor.execute("""
            ...
            (session_id, message_index, role, content, timestamp, token_count, model_used, embedding)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ...
        """, (message.session_id, message.message_index, message.role, message.content,
              message.timestamp, message.token_count, message.model_used, message.embedding))
        
        self.connection.commit()
        
        # Update FTS index
        cursor.execute("""
            INSERT INTO conversation_fts(rowid, content, role, session_id)
            VALUES (?, ?, ?, ?)
        """, (cursor.lastrowid, message.content, message.role, message.session_id))
        
        self.connection.commit()
        return cursor.lastrowid
    
    def query_conversation_memory(self, query: str, limit: int = 10, 
        ...
        if FTS_ENABLED:
            if session_id:
                cursor.execute("""
                    SELECT cm.*, cs.title, cs.workspace_path
                    FROM conversation_fts
                    JOIN conversation_messages cm ON conversation_fts.rowid = cm.id
                    JOIN conversation_sessions cs ON cm.session_id = cs.session_id
                    WHERE conversation_fts MATCH ? AND cm.session_id = ?
            ...
            if session_id:
                cursor.execute("""
                    ...
                    JOIN conversation_sessions cs ON cm.session_id = cs.session_id
                    WHERE cm.content LIKE ? AND cm.session_id = ?
                    ORDER BY cm.timestamp DESC
                    LIMIT ?
    ...
    def get_session_context(self, session_id: str, message_limit: int = 50) -> Dict[str, Any]:
    ...
    def store_conversation_insight(self, insight: 'ConversationInsight') -> int:
        """Store an extracted insight from conversation"""
        cursor = self.connection.cursor()
        
        tags_json = json.dumps(insight.tags) if insight.tags else None
        
        cursor.execute("""
            INSERT INTO conversation_insights
            (session_id, insight_type, content, context, importance, tags, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (insight.session_id, insight.insight_type, insight.content, insight.context,
              insight.importance, tags_json, insight.created_at or datetime.now()))

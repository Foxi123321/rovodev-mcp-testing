...
- _(internal)_ **`/tasks`** - List discovered tasks (Smart Tasks)
...
- _(internal)_ **`/mode`** - Switch between agent modes
...
- **`/help`** - Show help and available commands. For AI-powered help, use `/help <query>` (when enabled)
  - **`/<command> help`** - View detailed help for a specific command
- _(internal)_ **`/changelog`** - Display changelog and release notes
  - **`/changelog latest`** - Show only the latest version
  - **`/changelog <version>`** - Show a specific version (e.g., `/changelog v0.12.0`)
...
You can pass a prompt file using the `!./file.md` syntax in non-interactive mode. The file contents will be used as the instruction.

```bash
# Run with a prompt file
acli rovodev run "!./instructions.md"

# Combine a prompt file with additional instructions
acli rovodev "!./instructions.md Add unit tests for edge cases as well"
```
### Command-Line Flags
...
- **`--xid`** - Attach an Experience ID to tag requests and analytics
**Examples:**
...
# Tag runs with an Experience ID
acli rovodev run --xid "RDA-123-upgrade-auth-flow" "Update login flow for magic links"
```
## Example Prompts

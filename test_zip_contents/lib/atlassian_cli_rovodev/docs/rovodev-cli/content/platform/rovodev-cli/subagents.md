...
Choose whether the subagent should load default memory files:

- **Yes** (default): Load memory files (AGENTS.md, AGENTS.local.md) from the current workspace
- **No**: Do not load memory files

Memory files provide context about your project and workspace conventions. By default, subagents load these files to maintain consistency with the parent agent's understanding of your project.
...
Subagent system prompts support the `!` syntax to reference saved prompts. This allows you to reuse prompt content and keep your subagent definitions DRY (Don't Repeat Yourself).

### Using Prompt References

When writing your subagent's system prompt, use the `!prompt_name` syntax to include content from a saved prompt:

```
You are a code reviewer. Follow these guidelines:

!local_code_review

Make sure to provide constructive feedback.
```

The referenced prompt content will be automatically expanded and included in the system prompt when the subagent is invoked. References can be nested - if the referenced prompt contains its own `!` references, they will be recursively expanded as well.

### Built-in Prompts

Several built-in prompts are available for reference:
- `!commit` - Commit message generation guidelines
- `!local_code_review` - Local code review best practices
- `!improve-documentation` - Documentation improvement guidelines

Custom prompts must be defined in `.rovodev/prompts.yml` with a `name` field, and that name is what gets referenced in the `!` syntax (not the filename).
...
- Reference saved prompts using the `!` syntax (e.g., `!commit`) to include prompt content directly in your subagent's system prompt

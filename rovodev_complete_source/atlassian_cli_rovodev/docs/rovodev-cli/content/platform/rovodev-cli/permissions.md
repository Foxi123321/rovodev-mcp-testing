...
**Compound Commands:**
When the agent attempts to run compound bash commands (using `&&`, `||`, `;`, or pipes), each subcommand is evaluated independently. You'll be prompted for permission for each subcommand that hasn't been explicitly allowed.
...
2. **MCP schema/list calls**: Tool schema and listing calls are always allowed
3. **Exact command match**: For bash/PowerShell, checks for exact command match
4. **Pattern match**: For bash commands, checks for pattern matches (e.g., `git.*`)
5. **Tool-specific permission**: Checks the `tools` dictionary for the specific tool
6. **Default permission**: Falls back to the `default` permission setting
